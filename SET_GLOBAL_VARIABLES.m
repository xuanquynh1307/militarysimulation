...
global EnvironmentWidth ImageWidth SafeDistance AlignmentRange CohesionRange...
    ...
    ObstaclesNum Obstacles ObstaclesNumB ObstaclesB ObstaclesNumR ObstaclesBat ObstaclesR ObstaclesF ...
     Reds  Blues xi yi DameOfBlue DameOfRed Fights FightsNum BattleshipsNum Battleships ...
...
%Blues(:,1:2) = EnvironmentWidth*(2*rand([BluesNum,2])-1); % set random positionB
MaxLQHP = 120;
RedsLQ(:,1) = EnvironmentWidth/10*(rand([RedsLQNum,1]))-400; % set random position
...
% 13 max see ahead (for collision avoidance), 14 max avoid force (collision avoidance)
%}
Battleships = zeros(1,15);
Battleships(1,1) = 200; Battleships(1,2) = -300; 
...
%% List of Fights
Fights = zeros(3,15);
Fights(1,1) = 400; Fights(1,2) = -400; 
Fights(2,1) = 450; Fights(2,2) = -450; 
Fights(3,1) = 450; Fights(3,2) = -400; 
